...
+ (NSAttributedString *)zp_attachmentWithContent:(nullable id)content
    ...
    //memset(&callbacks,0,sizeof(CTRunDelegateCallbacks));//memset将已开辟内存空间 callbac
...
- (NSAttributedString *)zp_joinWithTruncationText:(NSAttributedString *)truncationText textRect:(CGRect)textRect maximumNumberOfRows:(NSInteger)maximumNumberOfRows {
    ...
    CFArrayRef lines = CTFrameGetLines(frame);// 在改frame下的文本有多少行，是一个c数组
    NSInteger numberOfLines = CFArrayGetCount(lines);/// 获取c数组中的个数，也就是真是的行数
    
    if (maximumNumberOfRows <= 0) {
        maximumNumberOfRows = numberOfLines;
    }
    
    if (numberOfLines > maximumNumberOfRows) {// 如果要显示的行数多余最大显示行数，一般而言需要折行显示，这里将行数设置为最大行数，然后将多余的CTLine删除，重新生成属性字符串
        numberOfLines = maximumNumberOfRows;
        
        NSInteger lastLineIndex = numberOfLines - 1 < 0 ? 0 : numberOfLines - 1;
        CTLineRef line = CFArrayGetValueAtIndex(lines, lastLineIndex);// 在c数组中找到对应索引下的CTLineRef
        CFRange lastLineRange = CTLineGetStringRange(line);// 找到对应字符串的range
